...
# Using the sf pacakge, turn the records in spatial points 
occ_sf <- st_as_sf(occ, coords = c("X", "Y"), crs = 4326)

leaflet(occ_sf) |>
  addTiles() |>  # optional — can remove if you don't want any base tiles
  addRasterImage(
    bioclim_data[[1]],
    colors = colorNumeric("viridis", values(bioclim_data[[1]]), na.color = NA),
    opacity = 0.6
  ) |>
  addLegend(
    pal = colorNumeric("viridis", values(bioclim_data[[1]]), na.color = NA),
    values = values(bioclim_data[[1]]),
    title = "Temperature"
  ) |>
  # Add IUCN polygon
  addPolygons(
...
bck_inside <- mask(bck, buffer(ahull_v, width=500000))
...
Randomly sampling points from inside or outside the study area does not account for the biases present in the occurrence data. Most occurrence points are found along roads or close to urban areas. One way of dealing with this bias in the occurence points, is to replicate the same biases in the background points. For example we could preferentially sample backgrounds points in places where people have sampled other closely related species of Agamid lizards. To do this we can create a density map of occurence records for the family.

```{r}
# read in distribution of all agamid species
agamid_occ <- read.csv("distribution/agamidae.csv")

# convert to spatVect object
agamid_occ <- vect(agamid_occ, geom=c("decimalLongitude", "decimalLatitude"))

# get density of records in grid cells using rasterize function
agamid_density <- rasterize(agamid_occ, bioclim_data[[1]], fun="sum" )

# plot it out - where are the highest density of occurences?
plot(log(agamid_density))
```

Now lets sample some background points accounting for this bias.

```{r}
# Sample 10,000 random points from across grid cells in the raster
bck_bias <- spatSample(bioclim_data[[1]], 10000, method = "stratified", weights=agamid_density, as.points = TRUE, na.rm = TRUE)

# Remove points that fall within the alpha hull 
bck_bias_outside <- erase(bck_bias, ahull_v)

# Remove points outside the alpha hull
bck_bias_inside <- mask(bck_bias, buffer(ahull_v, width=500000))
```
...
Plot different the distribution of a couple of different temperature and precipitation bioclim variables and make a note of how the presence and absence points occupy different parts of the climate space. Repeat for the two alternative kinds of background sampling methods.
...
Now that we have a set of uncorrelated predictor variables, we can define our models and fit them. Here the spline values is given as k = 4, for more complex curves you can increase this value, or for more simple curves you can reduce this.
...
par(mfrow=c(1,2))
plot(gam_prediction > gam_threshold_val, main="GAM binary")
plot(occ_sp, add=T, pch=16, cex=0.5)
plot(ahull_v, add=T)
...
-   **Question:** Which method provides a better representation of the species range? and how do they compare to our geometric SDMs from last week?

-   **Question:** What might we do to improve our SDM models?

------------------------------------------------------------------------

# **5**: SDM Predictions Through Time

If we're happy with our model in the present and convinced we have accurately modelled the niche of the species, we might want to ask how the species distribution has changed through time. Commonly, people look to model the change in distribution into the future using models of climate change under different emissions scenarios. Equally, we might also want to know how the species was distributed in the past to understand how population structure might have evolved over time when conditions were vastly different to today. One such period is the Last Glacial Maximum \~21,000 years ago. During the LGM, Australia looked very different, it was a colder and drier place than the present day and many species may have contracted their geographic ranges to warmer and more mesic refugia. Lets see what our SDM predicts for the distribution of the Frilled Lizard at the LGM!

```{r}
# load bioclim data for the LGM (from CHELSA)
lgm_bioclim_data <- rast("climate/chelsa_01_LGM.tif")
...
# first make lgm names lower case
names(lgm_bioclim_data) <- tolower(names(lgm_bioclim_data) )

# add in the prefix
names(lgm_bioclim_data) <- paste0("wc2.1_5m_", names(lgm_bioclim_data) )

# remove the 0 from bio1-9
names(lgm_bioclim_data)[1:9] <- gsub("0", "", names(lgm_bioclim_data)[1:9])

# add a underscore between bio and the number
 names(lgm_bioclim_data) <-  gsub("(bio)([0-9]+)", "\\1_\\2", names(lgm_bioclim_data))
...
Ok now we have data that represents the same variables, with matching names, at two different times. We can now predict the model on this new data.

```{r}
glm_prediction_lgm <- predict(lgm_bioclim_data, glm_sdm, type="response")
gam_prediction_lgm <- predict(lgm_bioclim_data, gam_sdm, type="response")


# Plot model predictions
par(mfrow=c(1,2))
plot(glm_prediction_lgm, main="glm")
plot(gam_prediction_lgm, main="gam")
...
# Apply threshold to get binary maps
par(mfrow=c(1,2))
plot(glm_prediction_lgm > glm_threshold_val, main="GLM binary")
plot(gam_prediction_lgm > gam_threshold_val, main="GAM binary")
```

-   **Question:** What kind of changes in the distribution of the species is predicted by the model? How could we validate this prediction?
